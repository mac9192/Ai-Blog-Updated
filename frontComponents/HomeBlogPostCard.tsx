...
const HomeBlogPostCard = ({post, layout = "vertical", reverse = false, locale }:PostProps) => {
    ...
    return (
        ...
        <div className="flex sm:flex-col md:flex-row p-3 md:mt-8 sm:mt-8">
                <div>
                    <Image className="max-h-[250px] md:max-w-[250px] " width={500} height={300} src={`${process.env.NEXT_PUBLIC_ASSETS_URL}${post.image}?key=optimized`} alt={post.title}  />
                    </div>
                    <div className="md:pl-5  sm:p-0 md:flex-col">
    <h2 className="font-bold tracking-wide sm:pt-5 md:pt-0 md:text-[24px]">{post.title}</h2>
